// function conclusion () {
//     $.get('cars.php', {token:localStorage['token']}, function loadPage() {
...
const carsCatalog = document.querySelector('#carsCatalog');
...
async function renderCars() {
    ...
    cars.forEach(car => {
        ...
        carNode.className = 'avto';
        carNode.style.alignItems = 'center';
        carNode.style.textAlign = 'center';
        carNode.innerHTML = `
            <img src="${car.IMG_URL}" alt="avto">
            <div>
                ${car.NAME} <br>
                Двигатель ${car.ENGINE_MODEL} <br>
                Мощностью ${car.POWER} л.с. <br>
                Цена аренды ${car.HOUR_PRICE} ₽/ч <br>
            </div>
            <a href="oplata.html?car_id=${car.ID}" class="text_block__button button">
                Арендовать
            </a>
        `;
        carsCatalog.appendChild(carNode);
    });
}
